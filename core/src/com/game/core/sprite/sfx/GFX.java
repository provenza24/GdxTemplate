...
public class GFX {
	...
	private static Color colourOne = new Color(255f / 255f, 128f / 255f, 0f / 255f, 1f);
	private static Color colourTwo = new Color(255f / 255f, 0f / 255f, 0f / 255f, 1f);
	...
	public static void drawSphereLightning(SpriteBatch batch, Vector2 point, float thickness, int numberOfBolts,
			int radius, int circleSegments) {
		drawSphereLightning(batch, point, thickness, numberOfBolts, radius, circleSegments, colourOne, colourTwo);
	}

	public static void drawSphereLightning(SpriteBatch batch, Vector2 point, float thickness, int numberOfBolts,
			int radius, int circleSegments, Color colourA, Color colourB) {
		for (int i = 0; i < 360f; i += (360f / circleSegments)) {
			tempSphereVector = getOrbitLocationDeg(point.x, point.y, i, radius);
			drawP2PLightning(batch, point.x, point.y, tempSphereVector.x, tempSphereVector.y,
					MathUtils.random(60f, 140f), MathUtils.random(0.8f, 3.8f), thickness, numberOfBolts, colourA,
					colourB);
		}
	}

	public static void drawChainLightningRandomBetweenPoints(SpriteBatch batch, Vector2 startPos, Vector2 endPointOne,
			Vector2 endPointTwo) {
		Vector2[] points = new Vector2[2];
		points[1] = startPos;
		points[0] = new Vector2(MathUtils.random(endPointOne.x, endPointTwo.x),
				MathUtils.random(endPointOne.y, endPointTwo.y));
		drawChainLightning(batch, points, thickness, numberOfBolts, colourOne, colourTwo);
	}

	public static void drawChainLightningRandomBetweenPoints(SpriteBatch batch, Vector2 startPos, Vector2 endPointOne,
			Vector2 endPointTwo, float thickness, int numberOfBolts) {
		Vector2[] points = new Vector2[2];
		points[1] = startPos;
		points[0] = new Vector2(MathUtils.random(endPointOne.x, endPointTwo.x),
				MathUtils.random(endPointOne.y, endPointTwo.y));
	...
	public static void drawChainLightningRandomBetweenPoints(SpriteBatch batch, Vector2 startPos, Vector2 endPointOne,
			Vector2 endPointTwo, float thickness, int numberOfBolts, Color colourA, Color colourB) {
		Vector2[] points = new Vector2[2];
		points[1] = startPos;
		points[0] = new Vector2(MathUtils.random(endPointOne.x, endPointTwo.x),
				MathUtils.random(endPointOne.y, endPointTwo.y));
	...
	private static void drawLine(SpriteBatch batch, float x1, float y1, float x2, float y2, float thickness,
			Texture tex) {
		...
		angle = angle - 180;
		batch.draw(tex, x1, y1, 0f, thickness * 0.5f, length, thickness, 1f, 1f, angle, 0, 0, tex.getWidth(),
				tex.getHeight(), false, false);
	}

	private static void drawP2PLightning(SpriteBatch batch, float x1, float y1, float x2, float y2, float displace,
			float detail, float thickness, int numberOfBolts, Color colourA, Color colourB) {
		batch.setBlendFunction(GL20.GL_SRC_ALPHA, GL20.GL_ONE);
		for (int i = 0; i < numberOfBolts; i++) {
			batch.setColor(MathUtils.random(colourA.r, colourB.r), MathUtils.random(colourA.g, colourB.g),
					MathUtils.random(colourA.b, colourB.b), 1f);
			drawSingleP2PLightning(batch, x1, y1, x2, y2, 117, 1.8f, thickness);
		}
		batch.setBlendFunction(GL20.GL_SRC_ALPHA, GL20.GL_ONE_MINUS_SRC_ALPHA);
	}

	private static void drawSingleP2PLightning(SpriteBatch batch, float x1, float y1, float x2, float y2,
			float displace, float detail, float thickness) {
		if (displace < detail) {
